...
client.on('message',function(message) {
  ...
  const swearWords = ["يا منتاك","يا قحبه","يا عرص","كس امك","كسمك"];
...
client.on("guildMemberAdd", member => {
  ...
  return channel.send(`Welcome TO Server :heart:
 ${member}  
 `) 
}).catch(console.error)
})
...
client.on("message", message => {
    if (message.content === (prefix + "help")) {
     const embed = new Discord.RichEmbed()
         ...
         ------------------------------
		 الاوامر العاديه
         @avatar:عرض الصورة الخاصه بك
		 @report:ابلاغ عن شخص ما
         @id : عرض ملفك الشخصي
         @server : احصائيات السيرفر
		 @verify:ليتفعيل نفسك
		 @link:يصنع لك رابط مؤقت للسيرفر
         ------------------------------
		 اوامر الادمن
         @ban : حظر العضو من السيرفر
         @clear : مسح الشات
		 @mute:تعطي ميوت لي شخص
		 @unmute:تفك الميوت عن شخص
		 @bc:يرسل رساله لي جميع اعضاء السيرفر
		 @createroles:يصنع لك جميع الرتب بالترتيب
         ------------------------------
         ليتفعيل خدمة اللوق برجاء صنع روم 
		 log
       **  `)
...
client.on('ready', () => {
   console.log(`----------------`);
    ...
    console.log(`---------------`);
  console.log(`Logged in as ${client.user.tag}!`);
  client.user.setGame(`| @help | Ǥolden $hop.System `,"http://twitch.tv/y04zgamer")
   client.user.setStatus("dnd")
});



client.login("NTM3MjcxMDI1NjM3Nzg1NjQw.Dyi0Sg.TOgpRh-1AarTdbCCBuJaWqdg-jM");